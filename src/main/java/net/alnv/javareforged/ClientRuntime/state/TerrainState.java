...
import net.alnv.javareforged.ClientRuntime.terrain.TerrainGraphArea;
...
public final class TerrainState {
    ...
    private final Map<RenderType, ObjectArrayList<SectionRenderDispatcher.RenderSection>> sectionsByLayer = new IdentityHashMap<>();
    private final SlotTerrainView view = new SlotTerrainView();
    @Nullable
    private TerrainGraphArea graphArea;
    private boolean graphReady;
    @Nullable
    private RenderType activeLayer;
    @Nullable
    ...
    public TerrainState() {
        for (RenderType renderType : RenderType.chunkBufferLayers()) {
            this.sectionsByLayer.put(renderType, new ObjectArrayList<>(10000));
        }
    }
    ...
    public TerrainGraphArea prepareGraph(
        ...
        if (this.graphArea == null || !this.graphArea.matches(store, this.view, level, viewDistance, slotId)) {
            this.graphArea = new TerrainGraphArea(
                store.dispatcher(),
                level,
                viewDistance,
                levelRenderer,
    ...
    public void onSectionCompiled(SectionRenderDispatcher.RenderSection section) {
        if (this.graphArea != null) {
            this.sectionOcclusionGraph.onSectionCompiled(section);
        }
    }
    ...
    public void rebuildLayerSections() {
        ...
        for (SectionRenderDispatcher.RenderSection section : this.visibleSections) {
            ...
            for (Map.Entry<RenderType, ObjectArrayList<SectionRenderDispatcher.RenderSection>> entry : this.sectionsByLayer.entrySet()) {
    ...
    public int layerSize(RenderType renderType) {
    ...
    public int nonEmptyVisibleCount() {
        int count = 0;
        for (SectionRenderDispatcher.RenderSection section : this.visibleSections) {
            if (!section.getCompiled().hasNoRenderableLayers()) {
                count++;
            }
        }
        return count;
    ...
    public void beginLayer(RenderType renderType) {
        if (this.activeLayer != null) {
    ...
    public void endLayer(RenderType renderType) {
