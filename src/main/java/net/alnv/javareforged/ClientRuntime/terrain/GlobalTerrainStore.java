...
import net.alnv.javareforged.Javareforged;
...
public final class GlobalTerrainStore {
    ...
    public DiagnosticCounts diagnoseView(
        int slotId,
        SlotTerrainView view,
        Iterable<SectionRenderDispatcher.RenderSection> visibleSections
    ) {
        Set<SectionRenderDispatcher.RenderSection> visibleSet = Collections.newSetFromMap(new IdentityHashMap<>());
        ...
        int noEntry = 0;
        int referenced = 0;
        int missingSlotReference = 0;
        int materialized = 0;
        int active = 0;
        int noSection = 0;
        int noOwner = 0;
        int retiring = 0;
        int ownerChunkLoaded = 0;
        int ownerChunkMissing = 0;
        int slotChunkLoaded = 0;
        int slotChunkMissing = 0;
        int visible = 0;
        int dirty = 0;
        int uncompiled = 0;
        int compiledEmpty = 0;
        int compiledNonEmpty = 0;
        int withActiveTasks = 0;
        ...
        for (TerrainKey key : view.sections()) {
            ...
            if (slotEntry.level == null) {
                missingSlotReference++;
            } else if (slotEntry.hasLoadedChunk()) {
                slotChunkLoaded++;
                ownerChunkLoaded++;
            } else {
                slotChunkMissing++;
            ...
            if (entry.ownerLevel != null && !entry.retiring) {
                active++;
            }
            if (visibleSet.contains(section)) {
                visible++;
            }
            if (section.isDirty()) {
                dirty++;
            ...
            if (compiled == SectionRenderDispatcher.CompiledSection.UNCOMPILED) {
                uncompiled++;
            } else if (compiled.hasNoRenderableLayers()) {
                compiledEmpty++;
            } else {
                compiledNonEmpty++;
            }
    ...
    public int estimateVisibleByExistingKeys(int slotId, SlotTerrainView view, Frustum frustum) {
        int count = 0;
        for (TerrainKey key : view.sections()) {
            Entry entry = this.entries.get(key);
            SlotEntry slotEntry = entry == null ? null : entry.slot(slotId);
    ...
    public ChunkLoadStats onChunkLoaded(ClientLevel level, net.minecraft.world.level.ChunkPos chunkPos) {
        ...
        for (int sectionY = level.getMinSection(); sectionY < level.getMaxSection(); sectionY++) {
            ...
            for (SlotEntry slotEntry : entry.slots) {
                ...
                referencedEntries++;
                slotRefsMask |= 1 << slotEntry.slotId;
                slotEntry.chunkAvailable = true;
                if (entry.section != null && entry.ownerLevel != null) {
                    entry.section.setDirty(false);
                    this.enqueueDirty(entry.section, true);
                    dirtied++;
                } else if (entry.section == null && slotEntry.level != null && !entry.retiring) {
                    this.enqueuePending(entry);
                    pending++;
                }
            }
    ...
    public ChunkLoadStats onChunkLoadedForSlot(int slotId, ClientLevel level, net.minecraft.world.level.ChunkPos chunkPos) {
        int referencedEntries = 0;
        int materialized = 0;
        int dirtied = 0;
        int pending = 0;
    ...
    public void deferDirty(SectionRenderDispatcher.RenderSection section) {
        if (this.isActive(section) && section.isDirty()) {
            this.enqueueDirty(section, section.isDirtyFromPlayer() || this.isRecentlyVisible(section));
        }
    }
    ...
    public ClientLevel ownerFor(SectionRenderDispatcher.RenderSection section) {
        ...
        this.updateOwner(entry);
        if (entry.ownerLevel == null) {
            throw new IllegalStateException("RenderSection has no ClientLevel owner: " + section.getOrigin());
        }
    ...
    public Vec3 cameraPositionFor(SectionRenderDispatcher.RenderSection section) {
        ...
        if (entry.retiring) {
            if (entry.retainedCameraPosition == null) {
                throw new IllegalStateException("Retiring RenderSection has no retained camera position: " + section.getOrigin());
            }
            return entry.retainedCameraPosition;
        }
        Vec3 cameraPosition = this.cameraPositionOrNull(entry, section);
        if (cameraPosition == null) {
            throw new IllegalStateException("RenderSection has no requesting camera position: " + section.getOrigin());
        }
    ...
    public double compilationPriority(SectionRenderDispatcher.RenderSection section) {
        ...
        Vec3 center = section.getOrigin().getCenter();
    ...
    public void taskStarted(SectionRenderDispatcher.RenderSection section) {
        Entry entry = this.requireEntry(section);
        if (entry.retiring) {
            throw new IllegalStateException("Terrain task started while section is retiring: " + section.getOrigin());
        }
        Vec3 cameraPosition = this.cameraPositionOrNull(entry, section);
        if (cameraPosition == null) {
            throw new IllegalStateException("Terrain task started before any requesting camera was captured: " + section.getOrigin());
        }
    ...
    public void taskFinished(SectionRenderDispatcher.RenderSection section) {
        ...
        if (remaining < 0) {
            throw new IllegalStateException("Terrain section finished more tasks than it started: " + section.getOrigin());
        }
        if (remaining == 0
            && !entry.retiring
            && (section.isDirty() || section.getCompiled() == SectionRenderDispatcher.CompiledSection.UNCOMPILED)) {
            section.setDirty(false);
            this.enqueueDirty(section, this.isRecentlyVisible(section));
        }
    }
    ...
    private void release(int slotId, TerrainKey key) {
        ...
        SlotEntry slotEntry = entry.slot(slotId);
        ClientLevel ownerBefore = slotEntry.level;
        SectionRenderDispatcher.RenderSection sectionBefore = entry.section;
        boolean retiringBefore = entry.retiring;
        int refMaskBefore = entry.referenceMask();
        ...
        this.recordReferenceChange("release", slotId, slotEntry, ownerBefore, sectionBefore, retiringBefore, refMaskBefore);
    }

    private void recordReferenceChange(
        String event,
        int slotId,
        SlotEntry slotEntry,
        @Nullable ClientLevel ownerBefore,
        @Nullable SectionRenderDispatcher.RenderSection sectionBefore,
        boolean retiringBefore,
        int refMaskBefore
    ) {
        ...
        int refMaskAfter = entry.referenceMask();
        TerrainDebug.recordReferenceDiagnostics(
            event,
            slotId,
            new ReferenceStats(
                entry.key,
                sectionBefore != null,
                sectionBefore != null && entry.section != sectionBefore,
                !retiringBefore && entry.retiring,
                ownerBefore != null && slotEntry.level == null,
                ownerBefore != null && entry.ownerLevel != null && ownerBefore != entry.ownerLevel,
                refMaskBefore,
                refMaskAfter,
    ...
    private SectionRenderDispatcher.RenderSection acquireSection(int blockX, int blockY, int blockZ) {
        ...
        return this.dispatcher.new RenderSection(index, blockX, blockY, blockZ);
    }
    ...
    public record DiagnosticCounts(
        int keys,
        int noEntry,
        int referenced,
        int missingSlotReference,
        int materialized,
        int active,
        int noSection,
        int noOwner,
        int retiring,
        int ownerChunkLoaded,
        int ownerChunkMissing,
        int slotChunkLoaded,
        int slotChunkMissing,
        int visible,
        int dirty,
        int uncompiled,
        int compiledEmpty,
        int compiledNonEmpty,
        int activeTasks
    ) {
        public boolean suspicious() {
            ...
            int minimumHealthyVisible = Math.min(96, Math.max(16, this.keys / 64));
            return this.visible < minimumHealthyVisible
                || this.noEntry > 0
                || this.missingSlotReference > 0
                || this.noSection > this.keys / 8
                || this.noOwner > 0
                || this.retiring > 0
                || this.ownerChunkMissing > 0
                || this.slotChunkMissing > this.keys / 8
                || (this.visible > 0 && this.compiledNonEmpty == 0)
                || (this.visible >= 32 && this.compiledEmpty > this.compiledNonEmpty * 2);
        }

        public String toLogLine() {
            return "keys=" + this.keys
                + " refs=" + this.referenced
                + " missingSlotRef=" + this.missingSlotReference
                + " noEntry=" + this.noEntry
                + " mat=" + this.materialized
                + " active=" + this.active
                + " noSection=" + this.noSection
                + " noOwner=" + this.noOwner
                + " retiring=" + this.retiring
                + " ownerChunk=" + this.ownerChunkLoaded + "/" + this.ownerChunkMissing
                + " slotChunk=" + this.slotChunkLoaded + "/" + this.slotChunkMissing
                + " visible=" + this.visible
                + " dirty=" + this.dirty
                + " compiled=" + this.uncompiled + "/" + this.compiledEmpty + "/" + this.compiledNonEmpty
                + " tasks=" + this.activeTasks;
        }
    }

    public record CompileStats(
        int candidates,
        int started,
        int completedEmpty,
        int skippedNotDirty,
        ...
        public boolean hasSignal() {
            return this.candidates > 0
                || this.skippedNoOwner > 0
                || this.skippedNoCamera > 0
                || this.skippedNoLight > 0
                || this.skippedInactive > 0
                || this.skippedActiveTasks > 0
                || this.allowanceBreak > 0;
        }

        public String toLogLine() {
            return "candidates=" + this.candidates
                + " started=" + this.started
                + " empty=" + this.completedEmpty
                + " skip=notDirty:" + this.skippedNotDirty
                + ",inactive:" + this.skippedInactive
                + ",noOwner:" + this.skippedNoOwner
                + ",noCamera:" + this.skippedNoCamera
                + ",noLight:" + this.skippedNoLight
                + ",activeTask:" + this.skippedActiveTasks
                + " cancelled=" + this.cancelledTasks
                + " allowanceBreak=" + this.allowanceBreak
                + " allowanceLeft=" + this.remainingAllowance
                + " pending=" + this.pending
                + " ownerWait=" + this.ownerWait
                + " dirtyQueues=" + this.dirtyQueues
                + " tasks=" + this.activeTasks;
        }
    }
    ...
    public record ChunkLoadStats(
        ...
        public boolean hasSignal() {
            return this.referencedEntries > 0 || this.materialized > 0 || this.dirtied > 0 || this.pending > 0;
        }

        public String toLogLine() {
            return "dim=" + this.dimension
                + " chunk=" + this.chunkX + "/" + this.chunkZ
                + " refs=" + this.referencedEntries
                + " mat=" + this.materialized
                + " dirty=" + this.dirtied
                + " pending=" + this.pending
                + " ownerWait=" + this.ownerWait
                + " slots=" + Integer.toBinaryString(this.slotRefsMask);
        }
    }
    ...
    public record ReferenceStats(
        ...
        int refMaskAfter,
        int referencesAfter,
        boolean slot0StillReferences,
        boolean anyNonZeroSlotReferences
    ) {
        public boolean hasSignal() {
            return this.removedSection
                ...
                || this.ownerLost
                || this.ownerChanged
                || (!this.slot0StillReferences && this.anyNonZeroSlotReferences);
        }

        public String toLogLine() {
            return "key=" + this.key
                + " hadSection=" + this.hadSection
                + " removedSection=" + this.removedSection
                + " retired=" + this.retired
                + " ownerLost=" + this.ownerLost
                + " ownerChanged=" + this.ownerChanged
                + " refs=" + Integer.toBinaryString(this.refMaskBefore) + "->" + Integer.toBinaryString(this.refMaskAfter)
                + " refsAfter=" + this.referencesAfter
                + " secondaryOnly=" + (!this.slot0StillReferences && this.anyNonZeroSlotReferences);
        }
    }
    ...
    private Entry requireEntry(SectionRenderDispatcher.RenderSection section) {
        Entry entry = this.sections.get(section);
        if (entry == null) {
            throw new IllegalStateException("RenderSection is not active: " + section.getOrigin());
        }
        return entry;
    }
