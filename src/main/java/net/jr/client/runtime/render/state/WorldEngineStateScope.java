...
import net.jr.client.runtime.bridge.GameRenderStateRuntimeAccessor;
...
public final class WorldEngineStateScope implements AutoCloseable {
    ...
    private final LevelRendererRuntimeAccessor renderer;
    @Nullable
    private final LevelExtractorRuntimeAccessor extractor;
    @Nullable
    private final GameRenderStateRuntimeAccessor gameState;
    @Nullable
    private final ClientLevel previousExtractorLevel;
    @Nullable
    private final ClientLevel previousParticleLevel;
    @Nullable
    private final LevelExtractionState previousExtractionState;
    @Nullable
    ...
    @Nullable
    private final ViewArea previousViewArea;
    private boolean closed;

    private WorldEngineStateScope(Minecraft minecraft, LocalClientSlot slot, boolean bindCompleteState) {
        this.minecraft = minecraft;
        this.slot = slot;
        this.bindCompleteState = bindCompleteState;
        this.renderer = (LevelRendererRuntimeAccessor)minecraft.levelRenderer;
        if (bindCompleteState) {
            this.extractor = (LevelExtractorRuntimeAccessor)minecraft.levelExtractor;
            this.gameState = (GameRenderStateRuntimeAccessor)minecraft.gameRenderer.gameRenderState();
    ...
    public static WorldEngineStateScope bind(Minecraft minecraft, LocalClientSlot slot) {
    ...
    public static WorldEngineStateScope bind(Minecraft minecraft, ClientLevel level) {
        ...
        return bind(minecraft, ClientRuntime.INSTANCE.slots().slot(slotId));
    }

    public static WorldEngineStateScope bindTerrainOnly(Minecraft minecraft, LocalClientSlot slot) {
        return new WorldEngineStateScope(minecraft, slot, false);
    }

    private void installSlot() {
        ClientRenderState state = this.slot.renderState();
    ...
    private static int slotIdForLevel(ClientLevel level) {
        for (int slotId = 0; slotId < LocalClientSlotRegistry.MAX_SLOTS; slotId++) {
            if (ClientRuntime.INSTANCE.slots().slot(slotId).renderState().level() == level) {
                return slotId;
            }
        }
        return -1;
