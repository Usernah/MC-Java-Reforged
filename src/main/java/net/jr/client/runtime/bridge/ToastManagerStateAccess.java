...
public final class ToastManagerStateAccess {
    ...
    public static Object nowPlayingToast(ToastManager manager) {
        return PASS_MODE.get() == PassMode.LOCAL
                ? null
                : GLOBAL_STATE.nowPlayingToast();
    }

    public static void setNowPlayingToast(
            ToastManager manager,
            @Nullable Object nowPlayingToast
    ) {
        GLOBAL_STATE.setNowPlayingToast(nowPlayingToast);
    }

    public static void bootstrapPrimary() {
        BOOTSTRAP_STATE.drainInto(
                ClientRuntime.INSTANCE.primarySlot().toastState()
        );
    }
        ...
        public void close() {
            ...
            if (current != this.installed) {
                throw new IllegalStateException(
                        "Toast pass scope is unbalanced: expected "
                                + this.installed
                                + ", found "
                                + current
                );
