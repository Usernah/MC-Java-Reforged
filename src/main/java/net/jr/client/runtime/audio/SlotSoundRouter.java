...
import net.jr.mixin.runtime.SoundEngineActiveSoundsAccessor;
import net.jr.mixin.runtime.SoundManagerEngineAccessor;
...
public final class SlotSoundRouter {
    private static final Logger LOGGER = LoggerFactory.getLogger(SlotSoundRouter.class);
    ...
    private static Integer currentSlotId() {
        ...
        return ClientRuntime.INSTANCE.clients().hasClient(0) ? 0 : null;
    }
    ...
    private static Integer routableActiveSlotId() {
        ...
        return ClientRuntime.INSTANCE.clients().hasClient(activeSlotId) ? activeSlotId : null;
    }

    private static void logFirstRoute(int slotId, SoundInstance sound, boolean transformSpatially) {
        if (!LOG_SOUND_ROUTING || !LOGGED_ROUTED_SLOTS.add(slotId)) {
            return;
        }
        LOGGER.info(
    ...
    private static boolean isClientAlive(int slotId) {
        return ClientRuntime.INSTANCE.clients().hasClient(slotId);
    }

    private static boolean isSpatialSlotAlive(int slotId) {
        if (!ClientRuntime.INSTANCE.clients().hasClient(slotId)) {
            return false;
        }
        LocalClientSlot slot = ClientRuntime.INSTANCE.slots().slot(slotId);
        return slot.renderState().level() != null && resolveCamera(slot) != null;
    }

    @Nullable
    private static Entity resolveCamera(LocalClientSlot slot) {
    ...
    private abstract static class RoutedSoundBase implements SoundInstance, SlotWrappedSoundInstance {
        ...
        protected boolean splitTest$slotCanPlay() {
            return this.transformSpatially ? isSpatialSlotAlive(this.slotId) : isClientAlive(this.slotId);
        }
